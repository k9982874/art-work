...
export const ProductListingSection = () => {
  const { state, loading, getAllSneakers } = useData();
  const {
    isProductInCart,
    isProductInWishlist,
    wishlistHandler,
  ...
  useEffect(() => {
    getAllSneakers();
  }, [ state.filters, state.inputSearch ]);

  const { allProductsFromApi, pagination } = state;
  ...
  return (
    <div className="product-listing-section">
      <div className="product-card-container">
        {(
          allProductsFromApi.map((product, index) => {
            const {
              _id,
              id,
              name,
